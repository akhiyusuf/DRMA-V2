...
export default function HomeClient({ homepage, products }: HomeClientProps) {
  ...
  // Hero title convention: text after the first ", " renders as the italic
  // second line. Falls back to a single line when there is no comma.
  const heroTitle: string = homepageData.hero?.title || "Modesty, Elevated";
  const [heroLine1, ...heroRest] = heroTitle.split(", ");
  const heroLine2 = heroRest.join(", ");

  // Mission section: only rendered when the CMS actually has content for it.
  // Title convention: " <br/> " splits the heading into two lines.
  const mission = homepageData.mission || {};
  const hasMissionContent = Boolean(
    (mission.title || "").trim() || (mission.description || "").trim() || mission.image
  );
  const [missionLine1, missionLine2] = (mission.title || "").split(" <br/> ");

  // Featured bento grid: hidden when no featured product resolves (e.g.
  // nothing configured in the CMS yet) instead of an empty section.
  const featuredProducts = (homepageData.featuredProductIds || [])
    .map((id: string) => products.find(p => p.id === id))
    .filter(Boolean) as Product[];

  // Differentiation: heading is optional — when the CMS has no title, the
  // section renders the points without a blank heading gap.
  const diff = homepageData.differentiation || {};
  const diffPoints: any[] = diff.points || [];
  const hasDiffTitle = Boolean((diff.title || "").trim());
  const [diffLine1, diffLine2] = (diff.title || "").split(" <br/> ");
    ...
    <div className="w-full bg-background overflow-hidden selection:bg-primary selection:text-primary-foreground">
      ...
      */}
      ...
      {/*
        ...
        Hidden entirely when the CMS has no mission content, so visitors
        never see an empty gray box + heading gap.
      */}
      {hasMissionContent && (
          ...
          <div className={`grid grid-cols-1 ${mission.image ? "md:grid-cols-2" : ""} gap-10 md:gap-16 lg:gap-24 items-center`}>
            ...
            <div className={`flex flex-col items-start w-full ${mission.image ? "" : "max-w-2xl mx-auto items-center text-center"}`}>
              ...
              >
                ...
                <Link
                  href={(mission.ctaUrl && mission.ctaUrl.trim() !== "" && mission.ctaUrl.trim() !== "#") ? mission.ctaUrl : "/ethics"}
                ...
                >
                  {mission.buttonLabel || "Learn More"}
                ...
                >
                  ...
                  <div className="relative flex-1 w-full bg-[#F3F3F1] rounded-[calc(1rem-0.25rem)] md:rounded-[calc(1.5rem-0.375rem)] lg:rounded-[calc(2rem-0.375rem)] overflow-hidden">
                    <img
                      ...
                      alt={product.name}
                      className={`absolute inset-0 w-full h-full object-cover transition-transform duration-[2.5s] ease-[0.32,0.72,0,1] group-hover:scale-[1.03] ${soldOut ? "opacity-70 saturate-50" : ""}`}
                    />
                    ...
                    {/* Sold Out badge — shown so visitors aren't sent to a
              ...
              >
                <div className="h-full p-5 md:p-8 lg:p-12 flex flex-col justify-between">
                  ...
                  {index === 2 && (
                    <Link
                      href="/shop"
                      className="group relative inline-flex items-center gap-3 rounded-full bg-background pl-5 pr-2 py-1.5 text-sm font-medium tracking-wide text-foreground transition-all active:scale-95 mt-6 md:mt-8 w-max focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring"
                    >
